...
if method == "shifted_rPCA":
    ...
    iter_ranks = [
        i
        for i in range(len(ret.ranks_hist[0]))
        if ret.ranks_hist[0][i] == nmodes[0] and ret.ranks_hist[1][i] == nmodes[1]
    ][0] + 1
    print("Figure 5: ", ret.ranks_hist, iter_ranks)
...
#     mu=mu,
# )
myparams = sPOD_Param(maxit=Niter, lamb=mu)
...
print("Figure 9a: ", ret.ranks_hist, iter_ranks)
...
# save_fig(pic_dir + "/convergence_ranks_sPOD_JBF.png", fig)
...
#     mu=mu,
# )
myparams_E = sPOD_Param(maxit=Niter, lamb=mu, mu=lambd0, isError=True)
ret_E = shifted_POD_BFB(qmat, trafos, nmodes, myparams_E)
iter_ranks = [
    i
...
# save_fig(pic_dir + "/convergence_ranks_shifted_sPOD_JBF_noise_.png", fig)
...
h = ax.semilogy(
    ...
    ret.rel_err_hist,
    "k-",
    label="sPOD_ADM-$\mathcal{J}_1$ $(r_1,r_2)=(%d,%d)$" % (nmodes[0], nmodes[1]),
)
...
# save_fig(pic_dir + "/convergence_sPOD_ADM_vs_sPOD_BFB.png", fig)
plt.show()
