...
from sPOD_tools import Frame, SVT, trunc_svd, shrink, trunc_SVT
...
def shifted_POD(snapshot_matrix, transforms, myparams, method, param_alm=None, nmodes=None, qt_frames=None, nmodesstat=0):
    ...
    if method == "ALM":
        ...
        return shifted_POD_ALMTV(
            snapshot_matrix, transforms, myparams, nmodes_max=nmodes, mu=param_alm, qt_frames=qt_frames
        )
        ...
        return shifted_POD_FBTV(
            snapshot_matrix, transforms, myparams, nmodes_max=nmodes, method="BFBTV"
        )
    elif method == "JFBTV":
        return shifted_POD_FBTV(
            snapshot_matrix, transforms, myparams, nmodes_max=nmodes, method="JFBTV"
        )
    elif method == "J2":
    ...
    elif method == "JFBTV_megaframe":
        return shifted_POD_FBTV_megaframe(snapshot_matrix, transforms, myparams, nmodes_max=nmodes, method="JFBTV_megaframe")
    ...
    elif method == "ALMTV_megaframe":
        return shifted_POD_ALMTV_megaframe(snapshot_matrix, transforms, myparams, nmodes_max=nmodes, mu=param_alm)
...
def shifted_POD_J2(
    ...
    while rel_err > myparams.eps and current_it < myparams.maxit:
        ...
        for k, (trafo, q_frame) in enumerate(zip(transforms, qtilde_frames)):
            ...
            if myparams.saveFrames:
                np.savetxt(myparams.outDir + "PSI_J2_%02d.dat"%k, U)
                np.savetxt(myparams.outDir + "sigma_J2_%02d.dat"%k, S)
                np.savetxt(myparams.outDir + "VT_J2_%02d.dat"%k, VT)
            rank = np.sum(S > 0)
            ranks[k] = rank
...
def shifted_POD_FB(
    ...
    while rel_err > myparams.eps and current_it < myparams.maxit:
        ...
        for k, (trafo, q_frame) in enumerate(zip(transforms, qtilde_frames)):
            ...
            S = q_frame.modal_system["sigma"]
            if myparams.saveFrames:
                np.savetxt(myparams.outDir + "PSI_%s_%02d.dat"%(method, k), q_frame.modal_system["U"])
                np.savetxt(myparams.outDir + "sigma_%s_%02d.dat"%(method, k), S)
                np.savetxt(myparams.outDir + "VT_%s_%02d.dat"%(method, k), q_frame.modal_system["VT"])
            rank = np.sum(S > 0)
            ranks[k] = rank
            ranks_hist[k].append(rank)
            qtilde += trafo.apply(q_frame.build_field())
...
def shifted_POD_ALM(snapshot_matrix, transforms, myparams, nmodes_max=None, mu=None, qt_frames=None):
    ...
    while rel_err > myparams.eps and it < myparams.maxit:
        ...
        for k, (trafo, q_frame) in enumerate(zip(transforms, qtilde_frames)):
            ...
            if myparams.saveFrames:
                np.savetxt(myparams.outDir + "PSI_ALM_%02d.dat"%(k), q_frame.modal_system["U"])
                np.savetxt(myparams.outDir + "sigma_ALM_%02d.dat"%(k), q_frame.modal_system["sigma"])
                np.savetxt(myparams.outDir + "VT_ALM_%02d.dat"%(k), q_frame.modal_system["VT"])
...
def shifted_POD_J2_megaframe(
    ...
    while rel_err > myparams.eps and it < myparams.maxit:
        ...
        if myparams.saveFrames:
            np.savetxt(myparams.outDir + "PSI_J2_megaframe.dat", umeg)
            np.savetxt(myparams.outDir + "sigma_J2_megaframe.dat", smeg)
            np.savetxt(myparams.outDir + "VT_J2_megaframe.dat", vtmeg)
        ...
        if nmodesstat > 0:
            ...
            if myparams.saveFrames:
                np.savetxt(myparams.outDir + "PSIstat_J2_megaframe.dat", ustat)
                np.savetxt(myparams.outDir + "sigmastat_J2_megaframe.dat", sstat)
                np.savetxt(myparams.outDir + "VTstat_J2_megaframe.dat", vtstat)
...
def shifted_POD_FB_megaframe(
    ...
    while rel_err > myparams.eps and it < myparams.maxit:
        ...
        if myparams.saveFrames:
            np.savetxt(myparams.outDir + "PSI_JFB_megaframe.dat", umeg)
            np.savetxt(myparams.outDir + "sigma_JFB_megaframe.dat", smeg)
            np.savetxt(myparams.outDir + "VT_JFB_megaframe.dat", vtmeg)        
        ...
        if myparams.stat_frame:
            ...
            if myparams.saveFrames:
                np.savetxt(myparams.outDir + "PSIstat_JFB_megaframe.dat", ustat)
                np.savetxt(myparams.outDir + "sigmastat_JFB_megaframe.dat", sstat)
                np.savetxt(myparams.outDir + "VTstat_JFB_megaframe.dat", vtstat)   
...
def shifted_POD_ALM_megaframe(
    ...
    while rel_err > myparams.eps and it < myparams.maxit:
        ...
        if myparams.stat_frame:
            ...
            if myparams.saveFrames:
                np.savetxt(myparams.outDir + "PSIstat_ALM_megaframe.dat", ustat)
                np.savetxt(myparams.outDir + "sigmastat_ALM_megaframe.dat", sstat)
                np.savetxt(myparams.outDir + "VTstat_ALM_megaframe.dat", vtstat)    
            rank_stat = np.sum(sstat>0)
        ...
        if myparams.saveFrames:
            np.savetxt(myparams.outDir + "PSI_ALM_megaframe.dat", umeg)
            np.savetxt(myparams.outDir + "sigma_ALM_megaframe.dat", smeg)
            np.savetxt(myparams.outDir + "VT_ALM_megaframe.dat", vtmeg)       
...
def shifted_POD_FBTV(
    ...
    if method == "BFBTV":
        ...
        print("Starting JFBTV")
    else: 
        print("Weird method -- results will be incorrect")
        input("Press any key to continue...")
    print('lambda_s = %f'%myparams.lambda_s)
    print('lambda_e = %f'%myparams.lambda_E)
    print('isError = ' + str(myparams.isError))
    ...
    while rel_err > myparams.eps and current_it < myparams.maxit:
        ...
        if myparams.isError:
            E = shrink(E + stepsize * res, stepsize * myparams.lambda_E)
            objective = (0.5 * norm(res, ord="fro") ** 2 
                + myparams.lambda_s * sum(norm(qk.build_field(), ord="nuc") for qk in qtilde_frames)
                + myparams.lambda_E * norm(E.flatten(), ord=1)
                + myparams.tv_mu *sum(norm((qk.build_field() @D).flatten(), ord=1) for qk in qtilde_frames))
        else:
            objective = (0.5 * norm(res, ord="fro") ** 2 
            + myparams.lambda_s * sum(norm(qk.build_field(), ord="nuc") for qk in qtilde_frames)
                + myparams.tv_mu *sum(norm((qk.build_field() @D).flatten(), ord=1) for qk in qtilde_frames))
    ...
    while rel_err > myparams.eps and current_it < myparams.maxit:
        ...
        for k, (trafo, q_frame) in enumerate(zip(transforms, qtilde_frames)):
            ...
            for subiter in range(N_U):
                ...
                U = U + step_U * G_U
                # Projection step
                U = U @ np.linalg.inv(np.real(sqrtm(U.T@U)))
            ...
            for subiter in range(N_S):
                G_S = khatri_rao(V,U).T@res_shifted.flatten(order='C')
            ...
            for subiter in range(N_V):
                ...
                V = V + step_V_prim*G_V - step_V_prim*step_V_dual*Z[k]@D.T
                V = V @ np.linalg.inv(np.real(sqrtm(V.T@V)))
                # 3.b) Dual update
                tmp = Z[k] + D@V
                Z[k] = tmp - shrink(tmp, myparams.tv_mu/step_V_dual)
...
def shifted_POD_FBTV_megaframe(
    snapshot_matrix,
    transforms,
    myparams,
    nmodes_max=None,
    method = "JFBTV_megaframe",
    nmodesstat = 0
):
    """
    This function implements the Forward-Backward method (FB).
    ...
    print("Starting JFBTV-megaframe")
    print('lambda_s = %f'%myparams.lambda_s)
    print('lambda_e = %f'%myparams.lambda_E)
    print('isError = ' + str(myparams.isError))
    ...
    while rel_err > myparams.eps and it < myparams.maxit:
        ...
        for k, trafo in enumerate(transforms):
            res_shifted = trafo.reverse(res)
            #qkhat = qmeg[:, k*nCols:(k+1)*nCols]
            qmeg[:, k*nCols:(k+1)*nCols] += tau * res_shifted 
            qmeg[:, k*nCols:(k+1)*nCols] -= tau*sigma*V[k]@D.T
            
        ###################################
        #      3. Proximal step           #
        ###################################
        [umeg, smeg, vtmeg] = trunc_SVT(qmeg, tau*myparams.lambda_s, nmodes_max, myparams.use_rSVD)
        ...
        for k, trafo_k in enumerate(transforms):
            ...
            qkhat = trafo_k.apply(qmeg[:, k*nCols:(k+1)*nCols])
            qhat.append(qkhat)
            qtilde += qkhat
        ...
        if myparams.isError:
            objective = (0.5*norm(res, ord='fro')**2
                + myparams.lambda_s * norm(qmeg, ord='nuc') 
                + myparams.lambda_E * norm(E.flatten(), ord=1) 
                + sum(myparams.tv_mu * norm(qhat[k]@D) for k in range(nTransports)))
        else:
            objective = (0.5*norm(res, ord='fro')**2
                + myparams.lambda_s * norm(qmeg, ord='nuc')
                + sum(myparams.tv_mu * norm(qhat[k]@D) for k in range(nTransports)))            
...
def shifted_POD_ALMTV(snapshot_matrix, transforms, myparams, nmodes_max=None, mu=None, qt_frames=None):
    """
    This function implements the Augmented Lagangian method (ALM).
    ...
    while rel_err > myparams.eps and it < myparams.maxit:
        ...
        for k, (trafo, q_frame) in enumerate(zip(transforms, qtilde_frames)):
            ...
            q_frame.set_orthonormal_system_svt(
                qk - tau*sigma*V[k]@D.T,
                mu_inv*tau * myparams.lambda_s
            )                                              #R_i without inertia
            # Dual update
            q_frame_field = q_frame.build_field()
            tmp = V[k] + q_frame_field @ D
...
def shifted_POD_ALMTV_megaframe(
    snapshot_matrix,
    transforms,
    myparams,
    nmodes_max=None,
    ...
    while rel_err > myparams.eps and it < myparams.maxit:
        ...
        for k, trafo_k in enumerate(transforms):
            rk = trafo_k.reverse(res)
            qmeg[:, k*nCols : (k+1)*nCols] += rk*tau
            qmeg[:, k*nCols : (k+1)*nCols] -= tau*sigma*V[k]@D.T
            
        # proximal step (6):
        [umeg, smeg, vtmeg] = trunc_SVT(qmeg, mu_inv *tau * myparams.lambda_s, nmodes, myparams.use_rSVD)
        rank_meg = np.sum(smeg > 0)
